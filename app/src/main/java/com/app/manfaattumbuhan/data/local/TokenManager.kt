...
object TokenManager {
    ...
    /**
     * Mendapatkan tingkat materi aktif yang harus dibaca secara berurutan (Sekuensial).
     * Aturan:
     * 1. Jika belum selesai baca materi "mudah", maka level aktifnya adalah "mudah".
     * 2. Jika sudah selesai "mudah" tapi belum "sedang" (dan level "sedang" atau lebih sudah terbuka), level aktifnya "sedang".
     * 3. Jika sudah selesai "mudah" dan "sedang", maka level aktifnya adalah "sulit" (jika terbuka) atau level tertinggi yang terbuka.
     */
    fun getActiveStudyLevel(userId: String): String {
        if (userId.isBlank()) return "mudah"
        val rawCurrent = getCurrentLevel(userId)?.lowercase() ?: "mudah"
        
        // Cek jika "mudah" belum dibaca
        if (!isMateriLevelDone(userId, "mudah")) {
            return "mudah"
        }
        
        val rawUnlocked = getUnlockedLevels(userId).map { it.lowercase() }.toSet()
        
        // Cek jika "sedang" belum dibaca, dan "sedang" atau "sulit" sudah terbuka
        if (!isMateriLevelDone(userId, "sedang") && 
            (rawUnlocked.contains("sedang") || rawUnlocked.contains("sulit") || hasEverReachedSulit(userId))
        ) {
            return "sedang"
        }
        
        // Cek jika "sulit" belum dibaca, dan "sulit" sudah terbuka
        if (!isMateriLevelDone(userId, "sulit") && 
            (rawUnlocked.contains("sulit") || hasEverReachedSulit(userId))
        ) {
            return "sulit"
        }
    ...
    fun levelToRank(level: String): Int = when (level.lowercase().trim()) {
        "mudah" -> 1
        "sedang" -> 2
        "sulit" -> 3
        else -> 0
    ...
    fun getMaxUnlockedRank(userId: String): Int {
        if (userId.isBlank()) return 0
        if (!isPretestDone(userId)) return 0
        if (hasEverReachedSulit(userId)) return 3
        val unlocked = getUnlockedLevels(userId)
            .map { it.trim().lowercase() }
            .filter { it == "mudah" || it == "sedang" || it == "sulit" }
            .toSet()
        val maxFromUnlock = unlocked.maxOfOrNull { levelToRank(it) } ?: 0
        return if (maxFromUnlock == 0) 1 else maxFromUnlock
    }
}
