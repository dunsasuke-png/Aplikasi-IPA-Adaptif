...
class DetailMateriFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnSudahBelajar.setOnClickListener {
            ...
            // Simpan status selesai untuk level saat ini
            TokenManager.setMateriLevelDone(userId, tingkatFromArg)
            updateStudiedButton(true)

            // Cek status sekuensial materi yang terbuka
            val maxRank = TokenManager.getMaxUnlockedRank(userId)
            val currentRank = TokenManager.levelToRank(tingkatFromArg)

            if (currentRank < maxRank) {
                // Masih ada level berikutnya yang terbuka namun belum dibaca (misal: Mudah selesai, Sedang belum)
                Toast.makeText(requireContext(), "Hebat! Mari lanjut membaca materi level berikutnya.", Toast.LENGTH_SHORT).show()
                findNavController().navigateUp() // Kembali ke MateriFragment, otomatis memuat level berikutnya
            } else {
                // Semua materi terbuka sudah selesai dibaca, lanjut ke halaman kuis
