...
class LatihanFragment : Fragment() {
    ...
    private fun showResultDialog(score: Int) {
        ...
        val messageHtml = """
            ...
            Waktu: ${String.format("%.0f", totalTime)} detik (rata-rata ${String.format("%.1f", kecepatanDetik)} detik/soal)<br/>
            <br/>
            Ketepatan: ${String.format("%.0f", ketepatan)}%<br/>
            Kecepatan: ${String.format("%.1f", kecepatanDetik)} detik/soal<br/>
        ...
        if (tingkat == "Pre-test") {
            ...
            // Setelah latihan, langsung kembali ke dashboard.
            // Level berikutnya tetap tersimpan, sehingga menu yang terbuka akan ikut update.
            dialogBuilder.setPositiveButton("Ke Beranda") { _, _ ->
                findNavController().navigate(R.id.action_latihan_to_dashboard)
            }
        }
