...
class MateriFragment : Fragment() {
    ...
    private fun normalizeLevelKey(level: String?): String? {
        val normalized = level?.trim()?.lowercase() ?: return null
        return when (normalized) {
            "mudah", "sedang", "sulit" -> normalized
            else -> null
        }
    }

    private fun levelToRank(level: String): Int = TokenManager.levelToRank(level)

    private fun rankToLevel(rank: Int): String = when (rank) {
        3 -> "sulit"
        2 -> "sedang"
        else -> "mudah"
    }

    private fun getMaxUnlockedRank(userId: String): Int = TokenManager.getMaxUnlockedRank(userId)

    private fun resolveInitialLevel(): String {
        val userId = TokenManager.getUserId()
        return TokenManager.getActiveStudyLevel(userId)
    }
    ...
    private fun isLevelUnlocked(level: String): Boolean {
        val userId = TokenManager.getUserId()
        val levelKey = normalizeLevelKey(level) ?: return false
        val maxRank = getMaxUnlockedRank(userId)
        
        val standardUnlock = levelToRank(levelKey) <= maxRank
        if (!standardUnlock) return false
        
        // Enforce sekuensial (berurutan):
        // Jika ingin mengakses "sedang", harus menyelesaikan "mudah" terlebih dahulu
        if (levelKey == "sedang" && !TokenManager.isMateriLevelDone(userId, "mudah")) {
            return false
        }
        // Jika ingin mengakses "sulit", harus menyelesaikan "sedang" terlebih dahulu
        if (levelKey == "sulit" && !TokenManager.isMateriLevelDone(userId, "sedang")) {
            return false
        }
    ...
    private fun loadCurrentLevel() {
        ...
        val maxRank = getMaxUnlockedRank(uid)
        val currentRank = levelToRank(currentLevel)
        if (currentRank == 0 || currentRank > maxRank) {
            currentLevel = rankToLevel(maxRank)
        }

        val levelLabel = currentLevel.replaceFirstChar { it.uppercase() }
        val levelIndex = levelToRank(currentLevel)

        binding.tvLevelTitle.text    = "Tingkat: $levelLabel"
        binding.tvPageIndicator.text = "Halaman $levelIndex dari $maxRank"
