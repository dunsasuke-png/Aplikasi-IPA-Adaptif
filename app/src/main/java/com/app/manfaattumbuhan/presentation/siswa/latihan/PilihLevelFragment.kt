...
class PilihLevelFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.cardMudah.setOnClickListener {
            val userIdStr  = TokenManager.getUserId()
            val unlockedTM = TokenManager.getUnlockedLevels(userIdStr)
            if (unlockedTM.contains("Mudah") || TokenManager.hasEverReachedSulit(userIdStr)) {
        ...
        binding.cardSedang.setOnClickListener {
            val userIdStr  = TokenManager.getUserId()
            val unlockedTM = TokenManager.getUnlockedLevels(userIdStr)
            if (unlockedTM.contains("Sedang") || unlockedTM.contains("Sulit") || TokenManager.hasEverReachedSulit(userIdStr)) {
        ...
        binding.cardSulit.setOnClickListener {
            val userIdStr  = TokenManager.getUserId()
            val unlockedTM = TokenManager.getUnlockedLevels(userIdStr)
            if (unlockedTM.contains("Sulit") || TokenManager.hasEverReachedSulit(userIdStr)) {
