...
class LatihanViewModel : ViewModel() {
    ...
    companion object {
        private const val QUESTIONS_PER_SESSION = 20
    }
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    ...
                    val filtered = soalList.shuffled().take(QUESTIONS_PER_SESSION)
    ...
    /** Returns how many questions haven't been answered yet. */
    fun getUnansweredCount(): Int {
        val total = getTotalSoal()
        if (total <= 0) return 0
        var count = 0
        for (i in 0 until total) {
            if (!answersMap.containsKey(i)) count++
        }
        return count
    }
    ...
    fun nextSoal() {
        ...
        if (current < list.size - 1) {
            ...
            // Last question: only finish if everything is answered
