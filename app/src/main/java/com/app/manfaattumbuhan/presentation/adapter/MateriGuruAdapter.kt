...
class MateriGuruAdapter(
    ...
    inner class ViewHolder(private val binding: ItemMateriGuruBinding) :
        ...
        fun bind(materi: MateriApi, displayNumber: Int) {
            val level = when (materi.tingkat.lowercase()) {
                "sedang" -> "Sedang"
                "sulit"  -> "Sulit"
                else     -> "Mudah"
            }
            binding.tvNamaMateri.text = materi.nama
            binding.tvDeskripsiMateri.visibility = android.view.View.GONE
            binding.tvUrutan.text = "Materi #$displayNumber • Level: $level"
